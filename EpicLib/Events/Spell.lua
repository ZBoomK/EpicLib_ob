...
function Player:RegisterListenedItemSpells()
  ...
  for _, Item in ipairs(UsableItems) do
    ...
    if Spell then
      -- EL.Print("Listening to spell " .. Spell:ID() .. " for item " .. TrinketItem:Name() )
...
function Player:RegisterListenedSpells(SpecID)
  ...
  local PlayerClass = EL.SpecID_ClassesSpecs[SpecID][1]
  -- Fetch registered spells during the init
  for Spec, Spells in pairs(EL.Spell[PlayerClass]) do
  ...
  if EL.Item[PlayerClass] then
    for Spec, Items in pairs(EL.Item[PlayerClass]) do
      for _, Item in pairs(Items) do
        local Spell = Item:OnUseSpell()
        if Spell then
          -- EL.Print("Listening to spell " .. Spell:ID() .. " for spec item " .. Item:Name() )
