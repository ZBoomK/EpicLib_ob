...
function Spell:RegisterAuraTracking()
  ...
  EL.Debug("RegisterAuraTracking " .. SpellID)
end

-- Unregister all tracked spells
function EL.UnregisterAuraTracking()
  EL.Debug("UnregisterAuraTracking()")
  ListenedAuras = {}
end

-- AddAuraToUnit
EL:RegisterForSelfCombatEvent(
  function(_, _, _, _, _, _, _, DestGUID, _, _, _, SpellID)
    ...
    if not AuraUnits[DestGUID] then
      ...
      -- EL.Print("AddAuraToUnit " .. SpellID .. " " .. DestGUID)
    else
      -- EL.Print("AddAuraToUnit Refresh " .. SpellID .. " " .. DestGUID)
      -- Refresh
    end
  end,
...
EL:RegisterForSelfCombatEvent(
  function(_, _, _, _, _, _, _, DestGUID, _, _, _, SpellID)
    ...
    if AuraUnits[DestGUID] then
      ...
      -- EL.Print("RemoveAuraFromUnit " .. Aura.Spell:Name() .. " " .. DestGUID)
    end
  end,
  "SPELL_AURA_REMOVED"
)
...
EL:RegisterForCombatEvent(
  function(_, _, _, _, _, _, _, DestGUID)
    for _, Aura in pairs(ListenedAuras) do
      ...
      if AuraUnits[DestGUID] then
        AuraUnits[DestGUID] = nil
        -- EL.Print("RemoveAurasFromUnit " .. Aura.Spell:Name() .. " " .. DestGUID)
      end
    end
  end,
...
do
  local function ScanAurasOnUnit(ThisUnit)
    ...
    if not GUID then
      -- EL.Print("ScanAurasForUnit - Invalid Unit")
      return
    end
    ...
    for _, Aura in pairs(ListenedAuras) do
      ...
      if ThisUnit:DebuffUp(ThisSpell, nil, true) or ThisUnit:BuffUp(ThisSpell, nil, true) then
        if not AuraUnits[GUID] then
          AuraUnits[GUID] = true
          -- EL.Print("ScanAurasForUnit - Adding " .. Aura.Spell:Name() .. " to unit " .. GUID)
        end
      else
        if AuraUnits[GUID] then
          AuraUnits[GUID] = nil
          -- EL.Print("ScanAurasForUnit - Removing " .. Aura.Spell:Name() .. " from unit " .. GUID)
        end
      end
    end
  end

  EL:RegisterForEvent(function(_, UnitID) ScanAurasOnUnit(Nameplates[UnitID]) end, "NAME_PLATE_UNIT_ADDED")
  EL:RegisterForEvent(function() ScanAurasOnUnit(Target) end, "PLAYER_TARGET_CHANGED")
end
...
do
  ...
  local function GetAuraUnit(Units, GUID)
    ...
    Units[GUID] = nil
    -- EL.Print("GetAuraUnit - Purging Unit " .. UnitGUID)
