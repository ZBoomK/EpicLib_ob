...
do
  local UserDefined = EL.GUISettings.General.Blacklist.UserDefined
...
do
  local CycleUserDefined = EL.GUISettings.General.Blacklist.CycleUserDefined
...
do
  local NotFacingExpireMultiplier = EL.GUISettings.General.Blacklist.NotFacingExpireMultiplier

  function Unit:IsFacingBlacklisted()
    if self:IsUnit(EL.UnitNotInFront) and GetTime() - EL.UnitNotInFrontTime <= Player:GCD() * NotFacingExpireMultiplier then
      return true
    end
    return false
  end
