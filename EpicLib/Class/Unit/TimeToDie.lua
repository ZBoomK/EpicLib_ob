...
function EL.FightRemains(Enemies, BossOnly)
...
function EL.BossFightRemains()
  return EL.FightRemains(nil, true)
end

-- Get if the Time To Die is Valid for a boss fight remains
function EL.BossFightRemainsIsNotValid()
  return EL.BossFightRemains() >= 7777
end

-- Returns if the current fight length meets the requirements.
function EL.FilteredFightRemains(Enemies, Operator, Value, CheckIfValid, BossOnly)
  local FightRemains = EL.FightRemains(Enemies, BossOnly)
...
function EL.BossFilteredFightRemains(Operator, Value, CheckIfValid)
  return EL.FilteredFightRemains(nil, Operator, Value, CheckIfValid, true)
end
