...
do
  local Spell = Class()
  EL.Spell = Spell
  function Spell:New(SpellID, SpellType, ReturnID)
    ...
    self.SpellType = SpellType or "Player" -- For Pet, put "Pet". Default is "Player". Related to EpicCache.Persistent.SpellLearned.
    self.SpellName = SpellName
    self.MinimumRange = MinimumRange
    self.MaximumRange = MaximumRange
    self.IsMelee = MinimumRange == 0 and MaximumRange == 0
    self.ReturnID = ReturnID or 0
    -- Variables
    self.LastCastTime = 0
...
do
  ...
  EL.MultiSpell = MultiSpell

  function MultiSpell:New(ReturnID, ...)
    local Arg = {...}
    self.SpellTable = {}

    for _, Spell in pairs(Arg) do
      if type(Spell) == "number" then
        Spell = EL.Spell(Spell, nil, ReturnID)
      end
...
do
  ...
  function Item:New(ItemID, ItemSlotIDs)
    ...
    self.ItemUseSpell = DBC.ItemSpell[ItemID] and EL.Spell(DBC.ItemSpell[ItemID]) or nil
