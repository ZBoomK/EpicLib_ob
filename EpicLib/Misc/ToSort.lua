...
function EL.GetInstanceInfo(Index)
  if Index then
...
function EL.GetInstanceDifficulty()
  return EL.GetInstanceInfo(3)
end

-- Get the time since combat has started.
function EL.CombatTime()
  return EL.CombatStarted ~= 0 and GetTime() - EL.CombatStarted or 0
end

-- Get the time since combat has ended.
function EL.OutOfCombatTime()
  return EL.CombatEnded ~= 0 and GetTime() - EL.CombatEnded or 0
end

-- Get the Boss Mod Pull Timer.
function EL.BMPullTime()
  if not EL.BossModTime or EL.BossModTime == 0 or EL.BossModEndTime - GetTime() < 0 then
    return 60
  else
    return EL.BossModEndTime - GetTime()
  end
end
...
do
  ...
  local Latency = 0
  local LatencyFrame = CreateFrame("Frame", "EpicLib_LatencyFrame", UIParent)
  ...
  function EL.RecoveryOffset(Bypass)
    if (Bypass) then return 0 end

    return Latency + EL.RecoveryTimer()
  end
end
